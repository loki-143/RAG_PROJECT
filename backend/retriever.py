...
class HybridRetriever:
    ...
    def retrieve(
        ...
        for repo_url in repo_urls:
            ...
            # If asking about a specific file, prioritize file matches
            if target_filename:
                file_matches = self._find_chunks_by_filename(
                    index_data['chunk_list'], 
                    target_filename
                )
                for chunk in file_matches:
                    # High score boost for filename matches
                    all_candidates[chunk.chunk_id] = (chunk, 2.0)
                    logger.info(f"Found file match: {chunk.source}")
    ...
    def _extract_filename_from_query(self, query: str) -> Optional[str]:
        ...
        # Look for patterns like "the X file" or "X.c file"
        file_ref_pattern = r'(?:the\s+)?([a-zA-Z_][a-zA-Z0-9_]*)\s+file'
    ...
    def _find_chunks_by_filename(
        ...
        for chunk in chunks:
            ...
            if (target_lower == source_basename or 
                target_lower == source_name_no_ext or
                source_basename.startswith(target_lower.split('.')[0])):
